$(document).ready(function () {
    ...
    function StartApplication(data) {
        ...
        autocompleteNames = [...new Set(HotelNames)];   //JS Destructuring
        ...
        for (var guestRating of GuestRatings) {
            ...
            if (guestRating == "Good") guestRatingDrop.append(`<option value="${guestRating}">Good 6 - 8</option>`);
        ...
        var moreFilterElements = Filters.map(x => `<option value="${x}">${x}</option>`);
        moreFiltersDrop.append(moreFilterElements);
        ...
        submitBtn.on('input', function () {   //Reset Button
            Controller();
        });
        ...
        hotelLocation = moreFiltersDrop.val();
